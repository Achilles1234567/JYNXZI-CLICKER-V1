...
<head>
<meta charset="UTF-8">
<title>Jynxzi Clicker ULTRA - Ultimate Edition</title>

<style>
body{
  margin:0;
  font-family:Arial;
  background:linear-gradient(270deg,#111,#1a1a1a,#222,#111);
...
.kuramaMode{
  background:radial-gradient(circle,#ff9800,#b71c1c,#000);
}
...
button:hover{ background:gold; transform:scale(1.1); }

#topNav{ display:none; background:#222; padding:10px; }
#gameScreen,#sofiaScreen,#narutoScreen{ display:none; padding-top:20px; }
...
#jynxzi{ width:150px; cursor:pointer; }
...
.rasengan{
  ...
  pointer-events:none;
  animation:spin 0.4s linear infinite, fadeOut 0.6s forwards;
}
...
<source src="https://cdn.pixabay.com/download/audio/2022/03/15/audio_4f0f8c7c55.mp3?filename=epic-action-113060.mp3" type="audio/mpeg">
</audio>

<div id="startScreen">
  <h1>🔥 JYNXZI CLICKER ULTRA 🔥</h1>
...
<div id="topNav">
  ...
  <button onclick="showSofia()">☠️ Sofia</button>
  <button onclick="showNaruto()" id="narutoTab" style="display:none;">🌀 Naruto</button>
</div>

<div id="gameScreen">
  <h2 id="welcome"></h2>
  <h2 id="cookieCount">0</h2>
  <h3 id="perSecond">0 per second</h3>
  <img id="jynxzi" src="https://i.ibb.co/2N1v6sk/cookie.png" onclick="clickCookie(event)">
  <p>Rebirths: <span id="rebirthDisplay">0</span></p>
...
<div id="sofiaScreen">
  <h2>☠️ Sofia Upgrades</h2>
  <div id="sofiaUpgrades"></div>
</div>

<div id="narutoScreen">
  <h2>🌀 Naruto Mode</h2>
  <div id="chakraBarContainer"><div id="chakraBar"></div></div>
  <button onclick="activateChakraMode()">Activate Chakra Mode</button>
  <div id="narutoUpgrades"></div>
</div>

<script>
let cookies=0,clickPower=1,multiplier=1,cookiesPerSecond=0;
let rebirths=0,rebirthMultiplier=1;
let secretNaruto=false;
let chakra=0,maxChakra=100,chakraMode=false,chakraCooldown=false;
...
/* NARUTO UPGRADES (20% more expensive) */
let narutoUpgrades=[
...
function activateChakraMode(){
 if(chakra>=100 && !chakraMode && !chakraCooldown){
   ...
   },20000); // 20 seconds active

   setTimeout(()=>{ chakraCooldown=false; },20000); // 20 seconds cooldown
 }
}
...
function rebirth(){
 if(cookies>=1000000){
  cookies=0; clickPower=1; cookiesPerSecond=0;
  rebirths++; rebirthMultiplier+=1;
  updateDisplay();
 }
}
